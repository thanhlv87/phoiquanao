...
export const AuthScreen: React.FC = () => {
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-8 animate-fade-in relative overflow-hidden">
            ...
            <div className="absolute top-[-10%] left-[-10%] w-[60%] aspect-square bg-brand-100/30 rounded-full blur-[100px] -z-10 animate-pulse"></div>
            <div className="absolute bottom-[-5%] right-[-5%] w-[50%] aspect-square bg-purple-100/30 rounded-full blur-[80px] -z-10"></div>

            <div className="w-full max-w-sm bg-white p-8 rounded-[3rem] shadow-2xl shadow-brand-100/50 border border-white">
                ...
                <div className="bg-slate-100 p-1 rounded-2xl flex mb-8">
                    <button
                        ...
                        className={`flex-1 py-3 text-[10px] font-black uppercase tracking-widest transition-all rounded-xl ${mode === 'login' ? 'bg-white text-brand-600 shadow-md' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        Đăng nhập
                    </button>
                    <button
                        onClick={() => setMode('signup')}
                        className={`flex-1 py-3 text-[10px] font-black uppercase tracking-widest transition-all rounded-xl ${mode === 'signup' ? 'bg-white text-brand-600 shadow-md' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        Đăng ký
                    </button>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="space-y-3">
                        <div className="relative group">
                            <Icon name="mail" className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 group-focus-within:text-brand-600 transition-colors" />
                            <input
                                type="email"
                                ...
                                required
                                className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:outline-none focus:ring-4 focus:ring-brand-500/10 focus:border-brand-600 transition-all text-sm font-semibold text-slate-900 placeholder-slate-400"
                            />
                        </div>
                        <div className="relative group">
                            <Icon name="lock" className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 group-focus-within:text-brand-600 transition-colors" />
                            <input
                                type="password"
                    ...
                    <button
                        type="submit"
                        disabled={loading}
                        className="w-full bg-brand-600 text-white font-black py-5 rounded-2xl shadow-lg shadow-brand-100 active:scale-95 transition-all disabled:opacity-50 flex items-center justify-center uppercase text-[10px] tracking-[0.2em]"
                    >
